...
@implementation CameraScrollingLayer

+(CCScene *) scene
{
	CCScene *scene = [CCScene node];
	CameraScrollingLayer *layer = [CameraScrollingLayer node];
...
-(id) init
{
	if ((self = [super init]))
	{
		...
		// Notice how the back button can be activated whenever you touch the upper right corner of the screen,
		// even though the "<< back" label may not be visible. CCMenu & CCCamera do not work well together.
...
{
	// Notice: background images do not need to change their positions anymore.
	...
	centerX = eyeX = clampf(centerX, 0.0f, _worldSize.width - _screenSize.width);
	centerY = eyeY = clampf(centerY, 0.0f, _worldSize.height - _screenSize.height);
