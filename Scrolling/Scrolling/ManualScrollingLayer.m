...
@implementation ManualScrollingLayer

+(CCScene *) scene
{
	CCScene *scene = [CCScene node];
	ManualScrollingLayer *layer = [ManualScrollingLayer node];
...
-(id) init
{
	if ((self = [super init]))
	{
		...
		// This uses the customized subclass of CCFollow with which you can make modifications to how the scrolling should behave.
		CGRect worldBoundary = CGRectMake(0, 0, _worldSize.width, _worldSize.height);
		[self runAction:[CCCustomFollow actionWithTarget:_player worldBoundary:worldBoundary]];
	}
	return self;
}
